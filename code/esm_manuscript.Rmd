...
mgmt_lmer_table <- esm_group_comp %>%
  select(method_longest, method_long, ref_data, depth, tidy_lmer, tidy_drop) %>%
...
flextable(mgmt_lmer_table)
write_csv(mgmt_lmer_table, here("figs", "ms_figs", "method_mgmt_lmer.csv"))
...
ggplot(esm_stock_compare_longer, aes(x=method_longest, y=soc, fill=label)) +
  geom_boxplot() +
  facet_grid(ref_data~depth, scales="free", labeller=labeller(depth=stock_depth_labels)) +
