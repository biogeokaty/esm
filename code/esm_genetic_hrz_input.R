...
n_p <- tibble(project=c("KansasState", "NCState", "WashingtonState", "UnivOfMinnesota", 
                        "Illinois", "UConn", "TexasA&MPt-1", "OregonState", "UTRGV"),
              name=list(n_ks, n_ncs, n_wash, n_minn, n_ill, n_uconn, n_tam1, n_osu, n_utrgv),
              pattern=list(p_ks, p_ncs, p_wash, p_minn, p_ill, p_uconn, p_tam1, p_osu, p_utrgv))

# Calculate horizon depth boundaries and reference soil masses for each horizon in each project ----

# Nest soil profile data
nested <- soc_horizon %>%
  group_by(project) %>%
  mutate(project_copy = project) %>%
  nest()

# Join soil data with generalized horizon label information
join <- nest_join(nested, n_p, by="project") %>%
  unnest(cols = c(n_p)) 
...
# Manually set reference depth masses - want all projects to have a horizon from 0-10 cm, and then the rest of the profile is based on the results of the regression model
...
utrgv_depth_df <- data.frame(project= "UTRGV",
                             top = c(0, 10, 30, 50, 80),
                             bottom = c(10, 30, 50, 80, 100))
...
depths_manual <- rbind(kansas_depth_df, ncs_depth_df, wash_depth_df, minn_depth_df, ill_depth_df, 
                       uconn_depth_df, tam1_depth_df, osu_depth_df, utrgv_depth_df) %>%
  group_by(project) %>%
  nest() %>%
  rename(depths = data)

# Join depth data with soil data, add vector of depth list
depths_df <- nest_join(join, depths_manual, by="project") %>%
