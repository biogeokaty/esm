...
ref_mass_df <- join %>%
  ...
  select(project, data, top, bottom, mass_agg_min, mass_agg_mean) %>%
  mutate(Upper_cm = -top,
         Lower_cm = -bottom) %>%
  select(-top, -bottom) %>%
  nest(ref_mass = c(Upper_cm, Lower_cm, mass_agg_min, mass_agg_mean))
...
### Reference mass from any treatment in project ----
dir.create(here("data_processed", "esm_input_revision")) # don't need to run this if folder already exists
...
map2(.x = project_list,
     ...
     .f = ~{
       ...
       write_xlsx(list("Concentrations" = conc,
                       "BD" = bd,
                       "Ref_soil_mass" = mass),
                  here("data_processed", "esm_input_revision", glue::glue(.x, "_", .y, ".xlsx")))
     })

### Reference mass from reference treatment only ----
project_stat2 <- esm_refonly_input %>% distinct(project, stat)
project_list2 <- project_stat %>% pull(project) %>% as.character
stat_list2 <- project_stat %>% pull(stat) %>% as.character

dir.create(here("data_processed", "esm_input_refonly_revision"))
...
map2(.x = project_list2,
     ...
     .f = ~{
       ...
       write_xlsx(list("Concentrations" = conc,
                       "BD" = bd,
                       "Ref_soil_mass" = mass),
                  here("data_processed", "esm_input_refonly_revision", glue::glue(.x, "_", .y, ".xlsx")))
     })
...
dir.create(here("data_processed", "esm_output_revision"))
...
map2(.x = project_list,
     ...
     .f = ~{
       ...
       input_file_name <- here("data_processed", "esm_input_revision", 
                               glue::glue(.x, "_", .y, ".xlsx"))
       
       # Name of the output directory
       output_directory_name <- here("data_processed", "esm_output_revision", glue::glue(.x, "_", .y))
...
# Create the big folder for the output
dir.create(here("data_processed", "esm_output_refonly_revision"))
...
map2(.x = project_list2,
     ...
     .f = ~{
       ...
       # Name of the input .xlsx file
       input_file_name <- here("data_processed", "esm_input_refonly_revision", 
                               glue::glue(.x, "_", .y, ".xlsx"))
       
       # Name of the output directory
       output_directory_name <- here("data_processed", "esm_output_refonly_revision", glue::glue(.x, "_", .y))
...
esm1_files <- list.files(here("data_processed", "esm_output_revision"), pattern = "ESM\\.csv$", recursive=TRUE, full.names=TRUE)
esm1_files_short <- list.files(here("data_processed", "esm_output_revision"), pattern = "ESM\\.csv$", recursive=TRUE)

# Make a list of the ESM2 output files
esm2_files <- list.files(here("data_processed", "esm_output_revision"), pattern = "ESM2\\.csv$", recursive=TRUE, full.names=TRUE)
esm2_files_short <- list.files(here("data_processed", "esm_output_revision"), pattern = "ESM2\\.csv$", recursive=TRUE)
...
## Reference mass from reference treatment only ----
# Make a list of the ESM1 output files
esm1_refonly_files <- list.files(here("data_processed", "esm_output_refonly_revision"), 
                                 pattern = "ESM\\.csv$", recursive=TRUE, full.names=TRUE)
esm1_refonly_files_short <- list.files(here("data_processed", "esm_output_refonly_revision"), 
                                       pattern = "ESM\\.csv$", recursive=TRUE)

# Make a list of the ESM2 output files
esm2_refonly_files <- list.files(here("data_processed", "esm_output_refonly_revision"), 
                                 pattern = "ESM2\\.csv$", recursive=TRUE, full.names=TRUE)
esm2_refonly_files_short <- list.files(here("data_processed", "esm_output_refonly_revision"), 
...
# Write csv
write_csv(esm_join, here("data_processed", "esm_standard_depths_revision.csv"))
