...
# write_csv(project_avg_climate, here("figs", "revision4_figs", "tablesupp1_site_clim.csv"))
```
...
![](esm_manuscript_revision4_files/figure-gfm/map-1.png)<!-- -->

``` r
# ggsave(here("figs", "revision4_figs", "figsupp1_map.png"), width=180, height=130, units = "mm")
```
...
# write_csv(site_pedon_table, here("figs", "revision4_figs", "table1_site_pedon_numbers.csv"))
```
...
![](esm_manuscript_revision4_files/figure-gfm/fig%202%20plot%20bulk%20density%20by%20depth%20increment-1.png)<!-- -->

``` r
# ggsave(here("figs", "revision4_figs", "fig2_bd.png"), width=160, height=120, units="mm", dpi=400)
# ggsave(here("figs", "revision4_figs", "fig2.pdf"), width=160, height=120, units="mm", dpi=400)
```
...
# write_csv(ref_mass_table, here("figs", "revision4_figs", "tablesupp2_ref_masses.csv"))
```
...
aov_soc_inc <- esm |>
  group_by(depth_std) |>
  nest() |>
  mutate(aov = purrr::map(data, ~aov(soc ~ method_longest*project, data = .x)),
...
aov_soc_inc_output <- aov_soc_inc |> 
  dplyr::select(depth_std, tidy_aov) |> 
  unnest(cols = tidy_aov) |> 
  mutate(sig = ifelse(p.value < 0.05, "significant", "not_sig"))

aov_soc_inc_letters <- aov_soc_inc |> 
...
fig2_inc_letters_position <- esm |>
  ...
  left_join(aov_soc_inc_letters, by=c("depth_std", "method_longest"))
```
...
![](esm_manuscript_revision4_files/figure-gfm/fig%20supp2%20aov%20and%20increment%20stock%20calculation%20method-1.png)<!-- -->

``` r
# ggsave(here("figs", "revision4_figs", "figsupp2_soc_inc_all.png"), width=120, height=100, units="mm", dpi=400)
```
...
mgmt_inc_aov_df <- esm |> 
  ...
  nest() |> 
  mutate(aov = purrr::map(data, ~aov(soc ~ label*project, data=.x)),
         tidy_aov = purrr::map(aov, broom::tidy),
         hsd = purrr::map(aov, TukeyHSD),
...
mgmt_inc_aov_tidy <- mgmt_inc_aov_df |> 
  select(method_longest, depth_std, tidy_aov) |> 
  unnest(cols = tidy_aov) |> 
  mutate(sig = ifelse(p.value < 0.05, "significant", "not_sig"))

mgmt_inc_aov_letters <- mgmt_inc_aov_df |> 
  dplyr::select(depth_std, method_longest, letters_df) |> 
  unnest(cols=letters_df)

mgmt_inc_aov_letters_position <- esm |>
  group_by(method_longest, depth_std) |>
  dplyr::summarize(max_soc = max(soc)) |>
  left_join(mgmt_inc_aov_letters, by=c("depth_std", "method_longest")) |>
...
ggplot(esm, aes(x=method_longest, y=soc, fill=label)) +
  ...
  geom_text(data=mgmt_inc_aov_letters_position, 
...
![](esm_manuscript_revision4_files/figure-gfm/fig%20supp3%20soc%20mgmt%20incremental%20aov%20letters-1.png)<!-- -->

``` r
# ggsave(here("figs", "revision4_figs", "figsupp3_soc_mgmt_inc.png"), width=160, height=130, units="mm", dpi=400)
```
...
dsoc_inc_aov_df <- esm_diff |> 
  ...
  nest() |> 
  mutate(aov = purrr::map(data, ~aov(diff_soc ~ project*method_longest, data=.x)),
         tidy_aov = purrr::map(aov, broom::tidy),
         hsd = purrr::map(aov, TukeyHSD),
         letters = purrr::map2(aov, hsd, ~multcompLetters4(.x, .y, reversed=TRUE)),
         letters_df = purrr::map(letters, .f = ~{
...
dsoc_inc_aov_tidy <- dsoc_inc_aov_df |> 
  dplyr::select(label, depth_std, tidy_aov) |> 
  unnest(cols=tidy_aov) |> 
  mutate(comparison = "dsoc_inc") |> 
  relocate(comparison)

dsoc_inc_letters_df <- dsoc_inc_aov_df |> 
  dplyr::select(label, depth_std, letters_df) |> 
  unnest(cols=letters_df)
...
![](esm_manuscript_revision4_files/figure-gfm/fig3%20dsoc%20incremental-1.png)<!-- -->

``` r
# ggsave(here("figs", "revision4_figs", "fig3_dsoc_inc.png"), width=180, height=160, units="mm", dpi=400)
# ggsave(here("figs", "revision4_figs", "fig3.pdf"), width=180, height=160, units="mm", dpi=400)
```
...
dsoc_inc_each_aov <- esm_diff |> 
  ...
  nest() |> 
  mutate(aov = purrr::map(data, ~aov(diff_soc ~ method_longest, data=.x)),
         tidy_aov = purrr::map(aov, broom::tidy),
         hsd = purrr::map(aov, TukeyHSD),
         letters = purrr::map2(aov, hsd, ~multcompLetters4(.x, .y, reversed=TRUE)),
         letters_df = purrr::map(letters, .f = ~{
...
dsoc_inc_each_aov_tidy <- dsoc_inc_each_aov |> 
  dplyr::select(project, label, depth_std, tidy_aov) |> 
  unnest(cols=tidy_aov) |> 
  mutate(comparison = "dsoc_inc") |> 
  relocate(comparison)
...
dsoc_inc_each_letters <- dsoc_inc_each_aov |> 
  dplyr::select(project, label, depth_std, letters_df) |> 
  unnest(cols=letters_df) |> 
...
purrr::map(.x = projects, 
           .f = ~{
             ...
             # ggsave(here("figs", "revision4_figs", "proj_dsoc_inc", glue::glue("suppfig_dsoc_inc_", .x, ".png")),
...
![](esm_manuscript_revision4_files/figure-gfm/dsoc%20incremental%20for%20each%20project-1.png)<!-- -->

    ## 
    ## [[2]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20incremental%20for%20each%20project-2.png)<!-- -->

    ## 
    ## [[3]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20incremental%20for%20each%20project-3.png)<!-- -->

    ## 
    ## [[4]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20incremental%20for%20each%20project-4.png)<!-- -->

    ## 
    ## [[5]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20incremental%20for%20each%20project-5.png)<!-- -->

    ## 
    ## [[6]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20incremental%20for%20each%20project-6.png)<!-- -->

    ## 
    ## [[7]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20incremental%20for%20each%20project-7.png)<!-- -->

    ## 
    ## [[8]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20incremental%20for%20each%20project-8.png)<!-- -->

    ## 
    ## [[9]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20incremental%20for%20each%20project-9.png)<!-- -->
...
![](esm_manuscript_revision4_files/figure-gfm/visually%20compare%20dSOC%20between%20esm%20methods-1.png)<!-- -->
...
dsoc_inc_error_aov_df <- esm_inc_error_sub |> 
  group_by(depth_std, label) |> 
  nest() |> 
  mutate(aov = purrr::map(data, ~aov(pct_error ~ project*stat_data, data=.x)),
         tidy_aov = purrr::map(aov, broom::tidy),
         glance_aov = purrr::map(aov, broom::glance),
         hsd = purrr::map(aov, TukeyHSD),
         pvals = purrr::map(hsd, ~.x$stat_data[, "p adj"]), # have to extract pvals to use multcompLetters
         letters = purrr::map(pvals, multcompLetters),
         letters_df = purrr::map(letters, .f = ~{
           data.frame(.x$Letters) |> 
             rownames_to_column() |> 
             rename(stat_data = rowname,
                    letter = 2)
         }) 
  )
...
dsoc_inc_error_sig <- dsoc_inc_error_aov_tidy |>
  filter(term == "stat_data" | term == "project") |> 
  filter(p.value < 0.05) |> 
  distinct(depth_std) |> 
  pull()
...
![](esm_manuscript_revision4_files/figure-gfm/fig4%20inc%20dsoc%20error%20plot-1.png)<!-- -->

``` r
# ggsave(here("figs", "revision4_figs", "fig4_dsoc_inc_error.png"), width=180, height=160, units="mm", dpi=400)
# ggsave(here("figs", "revision4_figs", "fig4.pdf"), width=180, height=160, units="mm", dpi=400)
```
...
```
    ...
    ## # A tibble: 64 × 7
    ## # Groups:   depth_std, label [8]
    ##    comparison label depth_std term                      df statistic p.value
    ##    <chr>      <fct>     <dbl> <chr>                  <dbl>     <dbl> <chr>  
    ##  1 dsoc_inc   Ref           5 project                    8    111.   <0.001 
    ##  2 dsoc_inc   Ref           5 method_longest             6      6.66 <0.001 
    ##  3 dsoc_inc   Ref           5 project:method_longest    48      0.91 0.636  
    ##  4 dsoc_inc   Ref           5 Residuals                301     NA    <NA>   
    ##  5 dsoc_inc   Ref          10 project                    8     97.0  <0.001 
    ##  6 dsoc_inc   Ref          10 method_longest             6      4.14 <0.001 
    ##  7 dsoc_inc   Ref          10 project:method_longest    48      2.29 <0.001 
    ##  8 dsoc_inc   Ref          10 Residuals                301     NA    <NA>   
    ##  9 dsoc_inc   Ref          30 project                    8     89.3  <0.001 
    ## 10 dsoc_inc   Ref          30 method_longest             6      2.43 0.026  
    ## # ℹ 54 more rows

``` r
# write_csv(anova_inc_table, here("figs", "revision4_figs", "tablesupp3_anova_inc.csv"))
```
...
dsoc_inc_error_each_aov_df <- esm_inc_error_all |> 
  group_by(project, label, depth_std) |> 
  nest() |> 
  mutate(aov = purrr::map(data, ~aov(abs_error ~ stat_data, data=.x)),
         tidy_aov = purrr::map(aov, broom::tidy),
         glance_aov = purrr::map(aov, broom::glance),
         hsd = purrr::map(aov, TukeyHSD),
...
purrr::map(.x = projects, 
           .f = ~{
             ...
             # ggsave(here("figs", "revision4_figs", "proj_dsoc_error_inc", glue::glue("suppfig_dsoc_error_inc_", .x, ".png")),
             #                                  width=160, height=140, units="mm", dpi=400)
           })
```
...
![](esm_manuscript_revision4_files/figure-gfm/dsoc%20inc%20error%20plot%20for%20each%20project-1.png)<!-- -->

    ## 
    ## [[2]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20inc%20error%20plot%20for%20each%20project-2.png)<!-- -->

    ## 
    ## [[3]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20inc%20error%20plot%20for%20each%20project-3.png)<!-- -->

    ## 
    ## [[4]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20inc%20error%20plot%20for%20each%20project-4.png)<!-- -->

    ## 
    ## [[5]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20inc%20error%20plot%20for%20each%20project-5.png)<!-- -->

    ## 
    ## [[6]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20inc%20error%20plot%20for%20each%20project-6.png)<!-- -->

    ## 
    ## [[7]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20inc%20error%20plot%20for%20each%20project-7.png)<!-- -->

    ## 
    ## [[8]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20inc%20error%20plot%20for%20each%20project-8.png)<!-- -->

    ## 
    ## [[9]]

![](esm_manuscript_revision4_files/figure-gfm/dsoc%20inc%20error%20plot%20for%20each%20project-9.png)<!-- -->
...
``` r
# write_csv(dsoc_bd_error_lm_table, here("figs", "revision4_figs", "tablesupp4_dsoc_bd_regression_table.csv"))
```
...
![](esm_manuscript_revision4_files/figure-gfm/fig%205%20dsoc%20error%20vs%20bd%20error%20fixed%20depth-1.png)<!-- -->

``` r
# ggsave(here("figs", "revision4_figs", "fig5_fd_dsoc_error_vs_bd_error.png"), width=160, height=120, units="mm", dpi=400)
# ggsave(here("figs", "revision4_figs", "fig5.pdf"), width=160, height=120, units="mm", dpi=400)
```
...
![](esm_manuscript_revision4_files/figure-gfm/fig%20s22%20dsoc%20error%20vs%20bd%20error%20for%20esm%20methods-1.png)<!-- -->

``` r
# ggsave(here("figs", "revision4_figs", "figsupp22_all_methods_dsoc_error_vs_bd_error.png"), width=180, height=160, units="mm", dpi=400)
```
...
![](esm_manuscript_revision4_files/figure-gfm/fig6%20plot%20actual%20mass%20vs%20depth-1.png)<!-- -->

``` r
# ggsave(here("figs", "revision4_figs", "fig6_interpolation_profiles.png"), width = 190, height = 140, units = "mm")
```
...
![](esm_manuscript_revision4_files/figure-gfm/plot%20soil%20profiles-1.png)<!-- -->
...
![](esm_manuscript_revision4_files/figure-gfm/plot%20soil%20profiles-2.png)<!-- -->
...
![](esm_manuscript_revision4_files/figure-gfm/cumulative%20mass%20vs%20depth-1.png)<!-- -->
...
![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soc%20vs%20depth-1.png)<!-- -->

    ## 
    ## [[2]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soc%20vs%20depth-2.png)<!-- -->

    ## 
    ## [[3]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soc%20vs%20depth-3.png)<!-- -->

    ## 
    ## [[4]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soc%20vs%20depth-4.png)<!-- -->

    ## 
    ## [[5]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soc%20vs%20depth-5.png)<!-- -->

    ## 
    ## [[6]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soc%20vs%20depth-6.png)<!-- -->

    ## 
    ## [[7]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soc%20vs%20depth-7.png)<!-- -->

    ## 
    ## [[8]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soc%20vs%20depth-8.png)<!-- -->

    ## 
    ## [[9]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soc%20vs%20depth-9.png)<!-- -->
...
![](esm_manuscript_revision4_files/figure-gfm/soil%20mass%20in%20each%20depth%20increment-1.png)<!-- -->

    ## 
    ## [[2]]

![](esm_manuscript_revision4_files/figure-gfm/soil%20mass%20in%20each%20depth%20increment-2.png)<!-- -->

    ## 
    ## [[3]]

![](esm_manuscript_revision4_files/figure-gfm/soil%20mass%20in%20each%20depth%20increment-3.png)<!-- -->

    ## 
    ## [[4]]

![](esm_manuscript_revision4_files/figure-gfm/soil%20mass%20in%20each%20depth%20increment-4.png)<!-- -->

    ## 
    ## [[5]]

![](esm_manuscript_revision4_files/figure-gfm/soil%20mass%20in%20each%20depth%20increment-5.png)<!-- -->

    ## 
    ## [[6]]

![](esm_manuscript_revision4_files/figure-gfm/soil%20mass%20in%20each%20depth%20increment-6.png)<!-- -->

    ## 
    ## [[7]]

![](esm_manuscript_revision4_files/figure-gfm/soil%20mass%20in%20each%20depth%20increment-7.png)<!-- -->

    ## 
    ## [[8]]

![](esm_manuscript_revision4_files/figure-gfm/soil%20mass%20in%20each%20depth%20increment-8.png)<!-- -->

    ## 
    ## [[9]]

![](esm_manuscript_revision4_files/figure-gfm/soil%20mass%20in%20each%20depth%20increment-9.png)<!-- -->
...
![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soil%20mass%20in%20each%20depth%20increment-1.png)<!-- -->

    ## 
    ## [[2]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soil%20mass%20in%20each%20depth%20increment-2.png)<!-- -->

    ## 
    ## [[3]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soil%20mass%20in%20each%20depth%20increment-3.png)<!-- -->

    ## 
    ## [[4]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soil%20mass%20in%20each%20depth%20increment-4.png)<!-- -->

    ## 
    ## [[5]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soil%20mass%20in%20each%20depth%20increment-5.png)<!-- -->

    ## 
    ## [[6]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soil%20mass%20in%20each%20depth%20increment-6.png)<!-- -->

    ## 
    ## [[7]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soil%20mass%20in%20each%20depth%20increment-7.png)<!-- -->

    ## 
    ## [[8]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soil%20mass%20in%20each%20depth%20increment-8.png)<!-- -->

    ## 
    ## [[9]]

![](esm_manuscript_revision4_files/figure-gfm/cumulative%20soil%20mass%20in%20each%20depth%20increment-9.png)<!-- -->
