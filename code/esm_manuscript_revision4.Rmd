---
title: "esm_manuscript_revision4"
...
project_plotting_order <- c("UConn", "Illinois", "KansasState", "UnivOfMinnesota", "NCState", 
                            "OregonStateJory", "OregonStateWoodburn","TexasAM", "WashingtonState")
...
esm_all <- read.csv(here("data_processed", "esm_revision3", "esm_standard_depths.csv")) |>
  ...
  mutate(depth_std = case_when(layer==1 ~ 5,
         ...
         project = case_when(project == "TexasA&MPt-1" ~ "TexasAM",
                             .default = project),
...
I calculated SOC stocks in 0-5 cm, 5-10 cm, 10-30 cm, and 30-60 cm depth increments via fixed depth and ESM methods. For the ESM calculations, I tested six different reference mass options:
...
* Maximum soil mass in the control group (Ref) in an individual DSP4SH project (ESM control, max)
...
bd <- horizon_data |>
  ...
  mutate(project = case_when(project == "TexasA&MPt-1" ~ "TexasAM",
                             .default = project)) |> 
...
The thicker, purple line shows the results of linear interpolation, aka "classical" ESM in which the soil profile is discretized and 1 mm layers are summed in order to reach the reference mass. Looks here like most of the "errors" associated with the cubic spline interpolation occur in the big jump in SOC mass in the deeper depths...honestly the cubic spline is more likely to be correct.
