...
```{r fig range of reference masses in each ESM depth}
mean_ref_mass <- esm %>%
  group_by(depth_std) %>%
...
purrr::map(.x = projects, 
           .f = ~{
             ...
             # ggsave(here("figs", "revision2_figs", "proj_dsoc_error", glue::glue("suppfig_dsoc_", .x, ".png")),
             #                                  width=120, height=100, units="mm", dpi=400)
           })
```
...
ggplot(fd_error, aes(x=bd_diff, y=inc_delta_soc_error)) +
  ...
  facet_wrap(~depth_std, labeller=labeller(depth_std = depth_labels2)) +
...
ggplot(error_methods_to_plot, aes(x=bd_diff, y=inc_delta_soc_error)) +
  ...
  facet_grid(method_longest~depth_std,
             labeller=labeller(depth_std = depth_labels2,
  ...
  scale_color_manual(values=pnw_palette("Sunset2", 9), name="Project",
                     labels = project_labels_esm) +  
  theme_katy()
# ggsave(here("figs", "revision2_figs", "figsupp12_all_methods_dsoc_error_vs_bd_error.png"), width=180, height=140, units="mm", dpi=400)
```
...
# first figure out which regressions are significant
dsoc_regressions <- esm_error_bd_error %>%
  group_by(method_longest) %>%
  nest() %>%
  mutate(lm = purrr::map(data, ~lm(delta_soc_error ~ true_delta_soc, data=.x)),
         lm_tidy = purrr::map(lm, tidy),
         lm_adjr2 = purrr::map(lm, .f = ~{
           glance(.x) %>%
             select(adj.r.squared)
...
dsoc_sig_reg <- dsoc_regressions %>%
  unnest(cols=c(lm_tidy, lm_adjr2)) %>%
  filter(term=="true_delta_soc") %>%
  filter(p.value < 0.05) %>%
  mutate(p_print = if_else(p.value < 0.001, "<0.001", paste0("=='", format(round(p.value, 3), nsmall = 3), "'"))) %>%
  mutate(reg_lab = paste0("~R^{2} == ", round(adj.r.squared, 2), "*`,`~p", p_print))

# plot error against true delta SOC
ggplot(esm_error_bd_error, aes(x=true_delta_soc, y=delta_soc_error)) +
  geom_point(aes(color=project)) +
  geom_hline(yintercept=0, linetype="dashed") +
  geom_vline(xintercept=0, linetype="dashed") +
  geom_smooth(data=esm_error_bd_error |>  filter(method_longest %in% sig_reg$method_longest),
                           method="lm", color="black", lwd=0.5, se=FALSE) +
  ...
  scale_color_manual(values=pnw_palette("Sunset2", 9), name="Project",
                     labels = project_labels_esm) +  
  theme_katy()
# ggsave(here("figs", "revision2_figs", "figsupp13_dsocerror_vs_truedsoc.png"), width=150, height=120, units="mm", dpi=400)
```
