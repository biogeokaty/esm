...
ggplot(esm, aes(x=method_longest, y=soc_cum, fill=method_longest)) +
  ...
  geom_text(data=fig2_letters_position, aes(y=max_soc_cum + 30, x=method_longest, label=letters), size=2) +
...
# ggsave(here("figs", "revision_figs", "fig3_soc_cum_all.png"), width=120, height=100, units="mm", dpi=400)
# ggsave(here("figs", "revision_figs", "fig3.pdf"), width=120, height=100, units="mm", dpi=400)
```
...
ggplot(esm, aes(x=method_longest, y=soc_cum, fill=label)) +
  ...
  theme_katy() +
  theme(axis.text.x=element_text(angle=45, hjust=1))
# ggsave(here("figs", "revision2_figs", "fig4_soc_mgmt_alt.png"), width=120, height=100, units="mm", dpi=400)
```
...
# ggsave(here("figs", "revision2_figs", "fig5_soc_diff_alt.png"), width=120, height=120, units="mm", dpi=400)
```
...
dsoc_error_letters_df <- esm_error |> 
  group_by(depth_std) |> 
  nest() |> 
...
dsoc_error_letters_df2 <- dsoc_error_letters_df |> 
  left_join(dsoc_error_quant, by=c("method_longest", "depth_std"))
...
ggplot(esm_error_sub, aes(x=method_longest, y=pct_error, fill=method_longest)) +
  ...
  labs(x="Calculation method", y=expression(Delta*"SOC percent error")) +
  facet_wrap(~depth_std, scales="free", labeller=labeller(depth_std=cum_depth_labels)) +
  scale_x_discrete(labels=method_labels) +
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1),
        legend.position="none")
# ggsave(here("figs", "revision2_figs", "dsoc_error.png"), width=120, height=120, units="mm", dpi=400)

# try for each project
purrr::map(.x = projects, 
           .f = ~{
             esm_error %>%
               ...
               geom_abline(intercept=0, slope=0, linetype="dashed", linewidth=0.3) +
                labs(x="Calculation method", y=expression(Delta*"SOC percent error"),
                    title=glue::glue("error by calculation method - ", .x)) +
               facet_wrap(~depth_std, labeller=labeller(depth_std=cum_depth_labels)) +
               scale_x_discrete(labels=method_labels) +
                 scale_y_continuous(labels = scales::percent) +
               scale_fill_paletteer_d("nationalparkcolors::Arches",
...
ggplot(esm_error_bd_cv, aes(x=cv_bd_treatment, y=pct_error)) +
  geom_point(aes(color=project)) +
  geom_abline(intercept=0, slope=0, linetype="dashed", linewidth=0.3) +
  geom_smooth(method="lm", color="navyblue") +
  facet_wrap(~method_longest) +
...
ggplot(esm_error_bd_cv2, aes(x=cv_bd_project, y=pct_error)) +
  geom_point(aes(color=project)) +
  geom_abline(intercept=0, slope=0, linetype="dashed", linewidth=0.3) +
  geom_smooth(method="lm", color="navyblue") +
  facet_grid(depth_std~method_longest) +
  scale_color_manual(values=pnw_palette("Sunset2", 9), name="Project") + 
  theme_classic()

# or boxplot of errors with x axis as project ordered by bulk density CV
# method x depth grid

ggplot(esm_error_bd_cv2, 
       aes(x=fct_reorder(project, cv_bd_project), y=pct_error, fill=method_longest)) +
  geom_boxplot() +
    scale_fill_paletteer_d("nationalparkcolors::Arches",
                          name="Calculation method") + 
...
ggplot(esm_error_bd_cv2, aes(x=cv_bd_project, y=pct_error)) +
  geom_point(aes(color=method_longest)) +
  geom_smooth(method="lm", color="navyblue") +
  facet_wrap(~depth_std) +
  scale_color_paletteer_d("nationalparkcolors::Arches",
                          name="Calculation method") + 
...
bd_error <- bd_depth |> 
  ...
  mutate(abs_bd_error = abs(depth_wt_bd - bd_mean_bau),
         pct_error_bd = abs_bd_error / bd_mean_bau)
```

```{r pct error dsoc vs pct error bd}
esm_error_bd_error <- esm_error_bd_cv2 |> 
  left_join(select(bd_error, dsp_pedon_id, depth_std, abs_bd_error, pct_error_bd), by=c("dsp_pedon_id", "depth_std")) |> 
  mutate(abs_true_delta_soc = abs(true_delta_soc))

ggplot(esm_error_bd_error, aes(x=pct_error_bd, y=pct_error)) +
  geom_point(aes(color=project)) +
  facet_wrap(~method_longest) +
  labs(x ="bulk density - sample percent difference from BAU mean", y = expression(Delta*"SOC percent error")) +
  scale_y_continuous(labels = scales::percent) +
  ...
  theme_katy()
# ggsave(here("figs", "revision2_figs", "figsupp3_pct_error_vs_bd_error.png"), width=120, height=85, units="mm", dpi=400)
```
...
```{r fig s3 percent error vs dsoc}
...
ggplot(augmented, aes(x=abs_true_delta_soc, y=pct_error)) +
  ...
  geom_text(data=models, aes(x=100, y=100, label = glue::glue("y = {round(models$a, 2)} x^{round(models$b, 2)}")),
             size=6/.pt) +
  labs(x = expression(paste("| ", Delta, "SOC with ESM(project, min) |")), y = expression(Delta*"SOC percent error")) +
  scale_y_continuous(labels = scales::percent) +
  facet_wrap(~method_longest, labeller = labeller(method_longest = method_labels2)) +
  scale_color_manual(values=pnw_palette("Sunset2", 9), name="Project",
                     labels = project_labels_esm) +  
  theme_katy()
# ggsave(here("figs", "revision2_figs", "figsupp3_pct_error_vs_dsoc.png"), width=120, height=85, units="mm", dpi=400)
